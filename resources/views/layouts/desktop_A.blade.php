...
<div class="wrapper">
  ...
  <aside class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <div class="sidebar">
      ...
      <div class="user-panel mt-3 pb-3 mb-3 d-flex">
        <div class="info">
          <a class="d-block">Witaj adminstratorze!</a>
        </div>
      </div>
        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item">
                <a href="{{ url('/new_user') }}" class="nav-link active" onclick="event.preventDefault(); document.getElementById('new_user-form').submit();">
                  <i class="fas fa-plus-circle nav-icon"></i> 
                  <p>
                    {{ __('Dodaj użytkownika') }}
                  </p>
                </a>
                <form id="new_user-form" action="{{ url('/new_user') }}" method="POST">
                  @csrf
                </form>
              </li>

              <li class="nav-item">
                <a href="{{ url('/select_user') }}" class="nav-link active" onclick="event.preventDefault(); document.getElementById('select_user-form').submit();">
                  <i class="fas fa-user-edit nav-icon"></i> 
                  <p>
                    {{ __('Zmiana danych') }}
                  </p>
                </a>
                <form id="select_user-form" action="{{ url('/select_user') }}" method="POST">
                  @csrf
                </form>
              </li>

              <li class="nav-item">
                <a href="{{ url('/list_users_tmtd') }}" class="nav-link active" onclick="event.preventDefault(); document.getElementById('list_users_tmtd-form').submit();">
                  <i class="fas fa-minus-circle nav-icon"></i> 
                  <p>
                    {{ __('Usuń użytkownika') }}
                  </p>
                </a>
                <form id="list_users_tmtd-form" action="{{ url('/list_users_tmtd') }}" method="POST">
                  @csrf
                </form>
              </li>
          ...
          <li class="nav-item">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item">
                <a href="{{ url('/new_publication') }}" class="nav-link active" onclick="event.preventDefault(); document.getElementById('new_publication-form').submit();">
                  <i class="fas fa-plus-circle nav-icon"></i> 
                  <p>
                    {{ __('Dodaj ogłoszenie') }}
                  </p>
                </a>
                <form id="new_publication-form" action="{{ url('/new_publication') }}" method="POST">
                  @csrf
                </form>
              </li>

              <li class="nav-item">
                <a href="{{ url('/change_publication') }}" class="nav-link active" onclick="event.preventDefault(); document.getElementById('change_publication-form').submit();">
                  <i class="fas fa-user-edit nav-icon"></i> 
                  <p>
                    {{ __('Zmiana ogłoszenia') }}
                  </p>
                </a>
                <form id="change_publication-form" action="{{ url('/change_publication') }}" method="POST">
                  @csrf
                </form>
              </li>

              <li class="nav-item">
                <a href="{{ url('/list_publication') }}" class="nav-link active" onclick="event.preventDefault(); document.getElementById('list_publication-form').submit();">
                  <i class="fas fa-minus-circle nav-icon"></i> 
                  <p>
                    {{ __('Usuń ogłoszenie') }}
                  </p>
                </a>
                <form id="list_publication-form" action="{{ url('/list_publication') }}" method="POST">
                  @csrf
                </form>
              </li>
