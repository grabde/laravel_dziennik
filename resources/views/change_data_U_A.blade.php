    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <div class="card card-primary">
          ...
          <!-- /.card-header -->
          <!-- form start -->
          <form id="userForm" action="/update_U" method="POST">
            <!-- /.card-header -->
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="class" class="col-md-4 col-form-label text-md-right form-control" required>
                                <option value="{{$class[0]->id_class}}">{{$class[0]->name}}</option>
                                @foreach ($list_class as $item)
                                  @if ($item->id_class != $class[0]->id_class)
                    ...
                    <tr>
                      ...
                      <td>
                          <select name="schedule" class="col-md-4 col-form-label text-md-right form-control" required>
                              @if ($class[0]->id_schedule != 0)
                                <option value="{{$class[0]->id_schedule}}">{{$class[0]->id_schedule}}</option>
                              @endif
                              @foreach ($list_schedule as $item)
                                @if ($item->id_schedule != $class[0]->id_schedule)
